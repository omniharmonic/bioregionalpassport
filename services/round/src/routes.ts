 ...
 * proposing needs `round:propose`, voting `round:vote`. Handlers re-check authority themselves, so they are
 * safe even when mounted without the `auth` gate.
 */
import { requireAuthority, type RouteRequest } from '@passport/service-kit';
import {
  addAdjustment,
  closeRound,
  createProposal,
...
export function createRoundRoutes(deps: RoundDeps): RoundRoute[] {
  const steward = (req: RouteRequest) => requireAuthority(req, STEWARD_SCOPE);
  return [
    {
      method: 'POST',
      path: '/rounds',
    ...
    },
    {
      ...
      path: '/rounds/:id',
      auth: 'none',
      handler: async (ctx, req) => ({ body: await getRound(ctx, idOf(req)) }),
    },
    {
      method: 'POST',
      path: '/rounds/:id/open',
    ...
    },
    {
      ...
      auth: 'authority:round:vote',
      handler: async (ctx, req) => {
        const session = requireAuthority(req, 'round:vote');
    ...
    },
    {
      ...
      handler: async (ctx, req) => {
        ...
        if (round.status !== 'published') steward(req);
    ...
    },
    {
      ...
      auth: 'none',
      handler: async (ctx, req) => {
        const { tally, ballots } = await getTally(ctx, idOf(req));
        return { body: { tally, ballots } };
      },
    },
    {
      method: 'GET',
      ...
      auth: 'none',
      handler: async (ctx: RoundContext, req) => ({ body: await verifyRound(ctx, deps, idOf(req)) }),
    },
  ];
}
