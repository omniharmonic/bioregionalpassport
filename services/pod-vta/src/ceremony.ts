...
import { issueAuthorities, type IssuedAuthorities } from './pep.js';
 ...
 * Note that it writes real rows (steward, event, member, VAC log) into the pod.
 */
export async function ceremonyBackHalf(ctx: VtaContext, deps: CeremonyDeps, opts: CeremonyOptions = {}): Promise<CeremonyResult> {
  ...
  if (!eventId) {
    ...
    const ev = await createEvent(ctx, d, convener.subject, {
      title: 'Attestation smoke',
  ...
  return {
    ok: true,
    detail: `Admitted ${applicant.did} at T1 via event ${eventId}.`,
    grant,
    ack,
    vacs,
    vwc,
