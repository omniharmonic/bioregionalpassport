...
export async function encryptPrivateKey(privateKey: Uint8Array, masterKey: string): Promise<string> {
  const key = await importKey(masterKey);
  const iv = globalThis.crypto.getRandomValues(new Uint8Array(12));
  const ct = new Uint8Array(await subtle().encrypt({ name: 'AES-GCM', iv }, key, Uint8Array.from(privateKey)));
...
export async function decryptPrivateKey(encrypted: string, masterKey: string, slug = 'this pod'): Promise<Uint8Array> {
  const key = await importKey(masterKey);
  ...
  try {
    plain = await subtle().decrypt(
      { name: 'AES-GCM', iv: Uint8Array.from(base64urlnopad.decode(ivPart)) },
      key,
      Uint8Array.from(base64urlnopad.decode(ctPart)),
    );
  } catch {
    throw new Error(
