/** Shared client-side fetch helper for the steward console's `/api/vta/*` and `/api/index/*` calls. */
...
/**
 * Fetches JSON from a pod-scoped service route, relying on the `passport_session`
 * cookie (sent automatically, same-origin) for the steward's authority. Throws
 * an `ApiError` (the server's `{code,message,hint?}`) on a non-2xx response.
 */
export async function stewardFetch<T>(path: string, init: RequestInit = {}): Promise<T> {
